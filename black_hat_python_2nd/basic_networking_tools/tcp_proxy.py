...
def proxy_handler(local_socket: socket.socket, remote_host, remote_port, receive_first):
...
def server_loop(local_host, local_port, remote_host, remote_port, receive_first):
    print('[*] Starting proxy at %s:%d' % (local_host, local_port))
    server = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    try:
        server.bind((local_host, local_port))
    except Exception as e:
        print('problem on bind: %r' % e)
        print('[!!] Failed to listen on %s:%f' % (local_host, local_port))
    ...
    while True:
        ...
        proxy_thread = threading.Thread(
            ...
            args=(local_socket, remote_host, remote_port, receive_first)
        )
        proxy_thread.start()
        time.sleep(0.01) # helps keeping CPU usage low
